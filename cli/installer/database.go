...
func (d *Database) createDbIfMissing(db string) error {
	...
	if err != nil {
		...
		if err := d.LoadSchema(db); err != nil {
			d.logger.Error("error loading schema", zap.Error(err))
			return err
		}
	}
...
func (d *Database) LoadSchema(db string) error {
	schemaPath := path.Join(d.appDir, "documentserver", "var-www", "onlyoffice", "documentserver", "server", "schema", "postgresql", "createdb.sql")
	if _, err := os.Stat(schemaPath); os.IsNotExist(err) {
		d.logger.Info("schema file not found, skipping", zap.String("path", schemaPath))
		return nil
	}
	d.logger.Info("loading schema", zap.String("path", schemaPath))
	return d.executor.Run("snap",
		"run", App+".psql",
		"-U", d.user,
		"-d", db,
